...
class DatosGeograficos(AgregacionRaiz):
    ...
    def crear_datos_geograficos(self, datos_geograficos: DatosGeograficos):
        ...
        self.agregar_evento(DatosGeograficosCreados(id_datos_geograficos=self.id, estado=self.estado.name, nombre_propiedad=self.nombre_propiedad))

    def aprobar_datos_geograficos(self):
        self.estado = ov.EstadoDatosGeograficos.APROBADA

        self.agregar_evento(DatosGeograficosAprobados(self.id, self.nombre_propiedad))

    def rechazar_datos_geograficos(self):
        self.estado = ov.EstadoDatosGeograficos.RECHAZADA

        self.agregar_evento(DatosGeograficosRechazados(self.id, self.nombre_propiedad))