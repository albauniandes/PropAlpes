...
from auditoria_test.modulos.companias.infraestructura.schema.v1.eventos import EventoPropiedadCreada, EventoDatosGeograficosCreados
...
def suscribirse_a_eventos_de_propiedad(app=None):
    cliente = None
    try:
        cliente = pulsar.Client(f'pulsar://{utils.broker_host()}:6650')
        consumidor = cliente.subscribe('eventos-propiedad-creada', consumer_type=_pulsar.ConsumerType.Shared,
                                       subscription_name='auditoria-sub-eventos-propiedad',
